...
void SalesmanPath::setPath(int* path){
	std::copy(path, path+this->size, this->path.begin());
	this->computeLength();
}

void SalesmanPath::setp_pp(double p_p){
	this->p_pp=p_p;
}
...
void SalesmanPath::computeLength(){
	...
	for(int i =0; i<size; i++)
	{
		...
		length+=this->cities[path[i]].distance(cities[path[(i+1)%size]]);
	}
	this->length=length;
}
