...
//Read initial configuration
  ...
  else 
  {
	...
	cout<<"La temperatura all'inizio vale "<<(2.0/3.0)*kin/npart<<endl;
  for (int i=0; i<npart; ++i)
  {
  ...
  std::fill(correlation, correlation+correlation_size, 0);
  std::fill(stima_corr, stima_corr+correlation_size, 0);
  
  vtail=(8.0*M_PI*rho)/(9*std::pow(rcut,9))-(8.0*M_PI*rho)/(3*std::pow(rcut,3));
  ptail=32.0*M_PI*rho*(1/(9*std::pow(rcut,9))-1/(3*std::pow(rcut,3)))/vol;
  Measure();
  cout<<"iv "<< iv<<endl;
...
//compute the self-correlation

	  if(dr<box/2){
	  //cout<<"Per due particelle di distanza:  "<<dr<<" incremento: "<<(int)std::floor(2*correlation_size*dr/(cbrt(vol)))<<endl;
	  //cout<<"Lato della scatole: "<<cbrt(vol)<<endl;
	  correlation[(int)std::floor(correlation_size*dr/(box/2))]+=2;
	  }
      if(dr < rcut)
      {
		...
		p += 48*(1.0/pow(dr,12) - 1.0/(2*pow(dr,6)));
      }
    }          
  }
  
  //for(int i =0; i<correlation_size;i++){cout<<correlation[i]<<" ";}
  //cout<<endl;
  //cout<<"in measure"<<walker<<endl;
  for (int i=0; i<npart; ++i) kin += 0.5 * (vx[i]*vx[i] + vy[i]*vy[i] + vz[i]*vz[i]);
  ...
  walker[ip] = rho*((2.0 / 3.0) * kin/(double)npart)+p/(3.0*vol)+ptail;
  return;
}
...
void Reset(int iblk) //Reset block averages
{
   ...
   if(iblk == 1)
   {
	  ...
	  std::fill(glob_corr_av2, glob_corr_av2+correlation_size, 0);
	  std::fill(glob_corr_av, glob_corr_av+correlation_size, 0);
   }


	
  std::fill(corr_av, corr_av+correlation_size, 0);
...
void Accumulate(void) //Update block averages
{
   ...
   for(int i = 0; i<correlation_size;++i){
	 corr_av[i]   = corr_av[i] + correlation[i];
   }
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, Corr;
   const int wd=20;
    ...
    Corr.open("output_corr.dat",ios::app);
	...
	//TODO: probably inefficient, the code run much slower
	for(int i =0; i<correlation_size;++i){
		stima_corr[i] = 3*corr_av[i]/(blk_norm*rho*npart*4*M_PI*(std::pow(deltaR*i+deltaR,3)-std::pow(deltaR*i,3)));
		glob_corr_av[i] += stima_corr[i];
		glob_corr_av2[i] += stima_corr[i]*stima_corr[i];
		err_corr[i]=Error(glob_corr_av[i],glob_corr_av2[i],iblk);
	}
...
//PRINT THE CORRELATION FUNCTION
	Corr << setw(wd) << iblk  <<  setw(wd); 
	for(int i =0; i<correlation_size;++i){
		 Corr<< "(" << stima_corr[i] << ","<< glob_corr_av[i]/(double)iblk << "," << err_corr[i] <<")"<<";";
	}
	Corr<<endl;
    //cout << "----------------------------" << endl << endl;
